...
const audioVolume = computed(() => {
  ...
  if (volume === null) {
    return store.state.settings.audioVolume
  }
  return volume
})
...
const multipliers = computed(() =>
  markets.value.map((marketKey: string) => {
...
onMounted(() => {
  ...
  _secondsAgoExampleTimeout = setInterval(() => {
    secondsAgoExample.value = `${ago(time)} ago`
  }, 1000) as unknown as number
})

onBeforeUnmount(() => {
  if (_secondsAgoExampleTimeout) {
    clearInterval(_secondsAgoExampleTimeout)
  }
})

function formatAmountFn(amount: number) {
