...
class AggregatorService extends EventEmitter {
  ...
  /**
   * Initialize backend configuration from environment (for historical data)
   */
  initializeBackendConfig() {
    ...
    if (backendUrl) {
      console.log(
        '[aggregatorService] Backend API configured for historical data:',
        {
          url: backendUrl,
      ...
      // Send backend config to worker (for historical data only)
      this.dispatch({
  ...
  /**
   * Check if backend is configured (for historical data or screener)
   */
